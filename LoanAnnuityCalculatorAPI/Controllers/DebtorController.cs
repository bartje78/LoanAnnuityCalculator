    ...
    public class DebtorController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateBalanceSheetLineItems(int balanceSheetId, [FromBody] List<BalanceSheetLineItemDto> lineItems)
        {
            try
            {
                ...
                // Remove all existing NON-auto-generated line items
                ...
                foreach (var dto in lineItems)
                {
                    ...
                    {
                        // For auto-generated items, only update if it's an external loan (editable)
                        bool isExternalLoan = dto.Label?.ToLower().Contains("external") == true || 
                                             dto.Label?.ToLower().Contains("1st lien") == true ||
                                             dto.Label?.ToLower().Contains("eerste hypotheek") == true;
                        
                        if (isExternalLoan && dto.Id > 0)
                        {
                            // Find and update the existing external loan line item
                            var existingItem = balanceSheet.LineItems.FirstOrDefault(li => li.Id == dto.Id);
                            if (existingItem != null)
                            {
                                existingItem.Amount = dto.Amount;
                                existingItem.Label = dto.Label ?? existingItem.Label;
                                existingItem.Notes = dto.Notes ?? existingItem.Notes;
                            }
                        }
                        // Skip other auto-generated items
                        continue;
                    }
